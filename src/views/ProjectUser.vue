<template>
  <div>
            ...
            <template v-for="item in pages.filter(page => page.type_page === 'page simple')">
               <tr v-bind:key="item.id">
                  ...
                  <td>
                     ...
                     <a @click="showModal2(item)" class="button is-black">  
                        <strong>Modifier</strong>
                     </a>
               ...
               <modal4 v-show="isModalVisible2" :item="selectedItem2" v-if="isModalVisible2" @close="closeModal2">
            ...
            <template v-for="item in pages.filter(page => page.type_page === 'interface')">
               <tr v-bind:key="item.id">
                  ...
                  <td>
                     ...
                     <a @click="showModal3(item)" class="button is-black">  
                        <strong>Modifier</strong>
                     </a>
                     <a @click="dropPage(item)" class="button is-black">  
                        <strong>Supprimer</strong>
               ...
               <modal5 v-show="isModalVisible3" :item="selectedItem3" v-if="isModalVisible3" @close="closeModal3">
   ...
   <div class="table3">
    ...
    <modal3 v-show="isModalVisible1" :item="project" v-if="isModalVisible1" @close="closeModal1">
...
export default {
   ...
   async created () {
      const idUser = this.$store.getters.getAuthUser;
      ...
      const temp = await axios.get('http://localhost:8000/api/'+idUser+'/projects/'+this.$route.params.id);
   ...
   methods: {
      ...
      closeModal2({value1, value2}) {
        ...
        console.log("page modifiée"+value2);
      ...
      async dropProject() {
         ...
         const response = await axios.delete('http://localhost:8000/api/'+id+'/projects/'+this.$route.params.id);    
